...
//     Postcondition: Adds the items of rhs to the lhs.
//
//
...
//     check that the index is a valid index (i.e., index is less than the sequence size), and then return the specified item.
//
//   double mean() const
...
namespace coen79_lab3 {
	...
	private:
		//invarients of these values
		//iterator must be greater than 0 to be valid and always points either at the current object or at nothing at all
		//whatever the iterator refers to determines what the current object is
		//the count is always >= 0 and increases when the number of items increases, and decreases when the number of items decreases
		//the array has size CAPACITY, so it can hold up to CAPACITY elements.  it only holds objects of type value_type and is initalized without any objects in it
